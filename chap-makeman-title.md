...
ここまで来たら再生産したものを自動的に配布することなど容易い。イメージはこうだ。

![配信工場](images/chap-makeman-title/factory.png)

今回作ったシステムに以下の次の口を用意してやればいい。
試すにあたりインフラはすぐ消すbotでも使用されたと言われるOracle Cloudを使用した。

### mp3を食わせる
ここは単純だ。WebAPIの口でも用意してペイロードで音源データを受け取ればいい。
なんならURLを渡すでもいい。youtubeあたりならオーディオデータを引き抜くAPIがpythonに用意されている。

ちなみにサンプルソースはない。ここは自動化しなくてもサーバに音源さえおけば動くので
構想だけ練って作らなかった。気が向いたら作る。

### 配信する
まずyoutubeのアカウントを用意しライブ配信の申請をする。
そして先に用意したOracle CloudのインスタンスにGUIとリモートデスクトップアプリを入れ、
OBS StudioやSimpleScreenRecorderなどの動画配信アプリを入れる。

あとは音楽再生ソフトを使い、自動生成した音源をただひたすらに再生するだけ。

### 試しに動かした
試しに配信したのがこれだ。
![自動配信デモ](images/chap-makeman-title/factory_demo.png)

配信しながら裏でヴェイパーウェイブを自動生成しては音楽再生ソフトにキューイングする、と言った感じ。  
無料インスタンスではスペックが低すぎて動画配信アプリが途中で落ちた。  
  
mp3を食わせてナイトコアやヴェイパーウェイヴにするような部分はPythonさえ動けばいいので  
サーバレスでクラウド化することも簡単にできると思う。  
  

## 真の音楽とは
こんなにもインスタントなものを**音楽**だなんてとても呼べない。
![真の音楽](images/chap-makeman-title/true_music.png?scale=5)
そう、音楽とはどこを切り取っても**カートコバーン**に行き着くのだ。

## 戯言

こう言ったリミックスの求心力をシステムに落とすにあたり**フラクタルと1/fゆらぎ**や**フォルマント周波数**とかその辺のキーワードが引っかかる気がするけどちょっとそこまで調べる元気がない。

配信時のサムネイルが面白くないので画像生成AIを噛ませたいが現状まだまだGPUなどマシンスペックが求められるので無料じゃ難しい。ControlNetとLoRA使ってずっとセンシティブに踊らせるみたいな動画を流しておきたい構想だけはある。